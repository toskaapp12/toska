...
| **N-13** | **Medium** | functions | **M-2 phone false-positive fix was incomplete — the server hold path still FPs on year/number lists.** Surfaced by the round-2 E2E (`e2e-round2.mjs`): a post *"we dated in 2019 2020 2021 2022 2023"* was **held** on staging. Root cause: there are *three* PII detectors. M-2 fixed `moderation.js`'s `containsNameOrIdentifyingInfo`, but `validatePost`/`validateReply` decide holds via `computePostFlagReason`/`computeReplyFlagReason` → `containsPII` → **`hasPhoneNumber` (index.js)**, which counted *total* digits ≥10 — so year lists, score/duration lists, any 10+ digits across short tokens false-positived as `personal_information` and got held. Real, user-facing on prod + staging (grief posts routinely list years/dates). **Fixed:** `hasPhoneNumber` now treats a phone as a *single* number — a contiguous 10+ digit run, or 10+ digits across ≤4 groups — not a list of many short numbers; crisis lines still excluded; spaced-digit obfuscation falls through to the strong `moderation.js` detector. 6 regression tests in `functions.test.js`; verified end-to-end via `computePostFlagReason`. | ✅ local + 68 functions tests green; ⚠️ needs deploy to prod+staging to clear the live FP |
